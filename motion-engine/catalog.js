...
export function renderCatalog() {
  ...
  return `## Scene types\n\n${scenes}\n\n## Reusable motion object library\n\n${stats.total} reusable objects across ${Object.keys(stats.categories).length} categories. The Director may reference object IDs in scene intent, composition, media_slots, motion_grammar, or object timelines; renderers own the exact drawing and lifecycle.\n\n${renderObjectCatalog()}\n\n## Timeline-level: objects\nOptional persistent object identities for props, charts, diagram nodes, connectors, and UI artifacts that must survive across beats: "objects": [{"id": "chart-main", "role": "proof", "ref": "bar_chart", "scene_id": "proof", "states": [{"state": "enter", "at": 1.2}, {"state": "settle", "at": 1.8}, {"state": "transform", "at": 3.2, "to": {"x": 0.64, "scale": 1.08}}, {"state": "exit", "at": 5.4}]}]. State order is enter -> settle -> transform/connect/emphasize -> exit; use this whenever a renderer or HyperFrames composition should animate the same object rather than redrawing a disconnected replacement.\n\n## Overlay events\n\n${events}\n\n## Transitions\n${transitions}\n\n## Timeline-level: chapters\nOptional persistent progress rail across the top: "chapters": [{"title": "Intro", "at": 0}, ...] (2-6 entries, short titles, at = chapter start time). Use for listicles, multi-step education, and any video with named sections. Omit for single-thought videos.`;
}
