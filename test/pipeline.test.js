...
test("plans premium product short contract with deterministic asset warnings", async () => {
  ...
  try {
    ...
    const hyperframesDesign = await readFile(path.join(out, "video/hyperframes/DESIGN.md"), "utf8");
    ...
    assert.equal(video.hyperframes.design, "video/hyperframes/DESIGN.md");
    ...
    assert.deepEqual(video.hyperframes.validate_command, ["npx", "hyperframes", "validate"]);
    assert.deepEqual(video.hyperframes.inspect_command, ["npx", "hyperframes", "inspect", "--json"]);
    assert.ok(video.hyperframes.quality_gate_commands.some((command) => command.join(" ") === "npx hyperframes inspect --json"));
    ...
    assert.match(hyperframesDesign, /## Style Prompt/);
    assert.match(hyperframesDesign, /## What NOT to Do/);
    assert.match(hyperframesDesign, /transition-masked scene changes/);
    ...
    assert.match(hyperframesHtml, /id="scene-transition-wipe" class="clip transition-wipe"/);
    assert.match(hyperframesHtml, /data-track-index="1" data-object-ids/);
    ...
    assert.match(hyperframesQualityChecklist, /Visual identity/);
    ...
    assert.match(hyperframesQualityChecklist, /npx hyperframes validate/);
    assert.match(hyperframesQualityChecklist, /npx hyperframes inspect --json/);
    ...
    assert.ok(hyperframesSfxManifest.copied_assets.some((asset) => ["default-sfx-pack", "generated-default-sfx"].includes(asset.alias)));
    ...
    assert.equal(hyperframesData.quality_handoff.summary.total, 6);
    assert.ok(hyperframesData.quality_handoff.checks.some((check) => check.gate === "Visual identity" && check.status === "pass"));
    ...
    await access(path.join(out, "video/hyperframes/DESIGN.md"));
