...
launchclip plan .launchclip/my-oss-tool --format short-30 --style ugc-demo-punchy --renderer local-ffmpeg --talking-head heygen
...
This still does not generate the real HeyGen talking-head footage. It produces a deterministic social-ready preview with the same script contract a future Remotion, HyperFrames, HeyGen, or product-videogen adapter can consume.
...
- Local ffmpeg rendering is intentionally dependency-light: it creates a vertical preview from repo facts, usage commands, optional UI media, captions, captured terminal evidence, and social-ready script beats. It is still not a substitute for a full Remotion/HyperFrames or avatar render.
